  ...
  methods: {
    ...
    request_and_load_answers: function request_and_load_answers() {
      ...
      this.$http.get("/api/questions/".concat(this.qid, "/answers")).then(function (r) {
        _this5.count_answ = {};
        _this5.answers = r.data.data; // to count subanswers

        _this5.answers.forEach(function (x) {
          if (x.parent_id) {
            _this5.sub_answers_exists = true;
            _this5.count_answ[x.parent_id] = (_this5.count_answ[x.parent_id] || 0) + 1;
          }

          if (_this5.is_can_answer && x.author.id === _this5.user_id) {
            _this5.is_can_answer = false;
          }
        });

        _this5.load_answers = false;

        if (scroll_to_id) {
          Vue.nextTick(function () {
            _this5.scrollTo(scroll_to_id);
          });
        }
      });
    },
    ...
    save: function save(upload_name) {
      ...
      this.put("/api/questions/".concat(this.qid, "/answers"), data, function (r) {
        _this7.answer_id = r.data.id;

        for (var i = 0; i < _this7.get_upload(upload_name).files.length; i++) {
          _this7.get_upload(upload_name).files[i].data = {
            id: _this7.answer_id
          };
        }

        _this7.get_upload(upload_name).active = true;

        if (_this7.is_form_send_done(upload_name)) {
          console.log('123');

          _this7.request_and_load_answers(_this7.answer_id);
        }

        _this7.set_form_success();
      });
    },
