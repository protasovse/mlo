...
var _vueMaskedInput = _interopRequireDefault(__webpack_require__(350));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var vueSmoothScroll = __webpack_require__(353);

Vue.use(vueSmoothScroll);
Vue.component('v-select', VueSelect.VueSelect);
Vue.component('file-upload', VueUploadComponent);
Vue.component('masked-input', _vueMaskedInput.default);
var app = new Vue({
...
var _template = _interopRequireDefault(__webpack_require__(349));
...
var _default = {
  ...
  computed: {
    ...
    is_require_name: function is_require_name() {
      return !this.completed.includes('name');
    },
    is_require_phone: function is_require_phone() {
      return !this.completed.includes('phone');
    },
    is_require_city: function is_require_city() {
      return !this.completed.includes('city');
    },
    is_require_form: function is_require_form() {
      return this.is_require_email || this.is_require_name || this.is_require_phone || this.is_require_city;
    }
  },
  watch: {
    is_form_send_done: function is_form_send_done() {
  ...
  mounted: function mounted() {
    ...
    this.$http.get('/api/rubric', {
      params: {
        'level': 0
      }
    }).then(function (r) {
      _this.base_options = r.data.data;

      _this.optionsInit();
    });
    this.$http.get('/api/city/ip').then(function (r) {
      if (r.data.success) {
        _this.city = r.data.data;
      } else {
        _this.city = _this.default_city[0];
      }
    });
    this.$http.get('/api/city/default').then(function (r) {
      _this.main_city = r.data.data;
      _this.options_city = _this.default_city.concat(_this.main_city);
      _this.options_city.tags = true;
    });
    this.$http.get('/api/user/check').then(function (r) {
      _this.is_authorized = r.data.success;

      if (_this.is_authorized) {
        _this.name = r.data.data.first_name;

        if (_this.name) {
          _this.completed.push('name');
        }

        _this.phone = r.data.data.phone;

        if (_this.phone) {
          _this.completed.push('phone');
        }

        _this.email = r.data.data.email;

        if (_this.email) {
          _this.completed.push('email');
        }

        _this.city = {
          'id': r.data.data.city['id'],
          'name': r.data.data.city['name']
        };

        if (_this.city) {
          _this.completed.push('city');
        }
      }
    });
  },
  methods: {
    cityInptut: function cityInptut(payload) {
    ...
    getCity: function getCity(search, loading) {
      var _this2 = this;

      this.$http.get('/api/city/search', {
        params: {
          'keyword': search
        }
      }).then(function (r) {
        _this2.options_city = r.data.data;
      });
    },
    onSearch: function onSearch(search, loading) {
      ...
      this.$http.get('/api/rubric', {
        ...
        _this3.options = r.data.data;
      });
    },
    onInputRubric: function onInputRubric() {
      if (!this.rubric) {
        this.rubric = 'не выбрано';
      }
    },
    optionsInit: function optionsInit() {
      this.options = this.base_options; //if (this.rubric.length > 1 &&  this.rubric[0] === 'не выбрано') {
      //    this.rubric.shift()
      //}
    },
    ...
    save_paid: function save_paid() {
      this.is_paid_question = true;
      this.save();
    },
    save_free: function save_free() {
    ...
    save: function save() {
      ...
      try {
        ...
        this.put('/api/question', data, function (r) {
          ...
          for (var i = 0; i < _this4.$refs.upload.files.length; i++) {
            _this4.$refs.upload.files[i].data = {
              id: _this4.question_id
            };
          }

          _this4.$refs.upload.active = true;
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_inputmask_core__ = __webpack_require__(351);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__ff_polyfill__ = __webpack_require__(352);
